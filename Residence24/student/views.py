...
def residence_application(request):
    ...
    try:
        ...
        if not (student.first_name and student.last_name
                and student.home_address_street):
            messages.warning(
                request,
                "Please complete your profile before applying for residence.")
            return redirect('student:create_profile'
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            student_application.faculty = form.cleaned_data[
                'faculty']  # Optionally allow updating the faculty field
    ...
    return render(request, 'student/residence_application.html', {
        'form': form,
        'user': request.user
    })
...
def create_profile(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            student.student_ID = request.user.student_id  # Ensure the correct student ID is linked
            student.save()
...
def select_residence(request):
    try:
        ...
        application_status = application.status  # Assuming 'status' is a field in Application model

        # Handle different application statuses
        if application_status == "Declined":
            return render(request, 'student/select_residence.html',
...
def submit_residence_choice(request):
    if request.method == 'POST':
        ...
        messages.success(request,
                         'Your residence choice has been saved successfully.')
...
def residence_details(request):
    try:
        ...
        residence_application = Application.objects.get(
            student=request.user.student)
        residence = residence_application.residence  # Access the related residence through the application
    except Application.DoesNotExist:
        residence = None

    return render(request, 'student/residence_details.html',
                  {'residence': residence})
