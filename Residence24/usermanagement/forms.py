...
class RegisterStudentForm(forms.ModelForm):
    password1 = forms.CharField(widget=forms.PasswordInput(attrs={'placeholder': 'Create a password', 'required': 'required'}))
    password2 = forms.CharField(widget=forms.PasswordInput(attrs={'placeholder': 'Confirm your password', 'required': 'required'}))
    ...
    def save(self, commit=True):
        ...
        if commit:
            ...
            HousingAdmin.objects.create(
                ...
                cell_number=self.cleaned_data['cell_number']  # Use cleaned data for cell number
            )
        return user
...
class StudentLoginForm(forms.Form):
    email = forms.EmailField(widget=forms.EmailInput(attrs={'placeholder': 'Enter your DUT email', 'class': 'form-control'}))
    ...
    def clean(self):
        ...
        if email and password:
            ...
            if user is None:
                ...
                raise forms.ValidationError("This account is not a housing admin account.")
            elif not user.is_active:
                raise forms.ValidationError("This account is inactive.")
