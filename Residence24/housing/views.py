...
def manage_residences(request):
        ...
        if 'edit_residence' in request.POST:
            ...
            if form.is_valid():
                ...
                for _ in range(residence.total_num_of_rooms):
                    room_type = random.choice(Room.ROOM_TYPE_CHOICES)[0]  # Choose random room type
    ...
    return render(request, 'housing/manage_residences.html', {
        'form': form,
        'residences': residences,
    })
...
def add_room(request, residence_id):
    try:
        residence = Residence.objects.get(residence_ID=residence_id)  # Ensure you're using residence_ID here
    ...
    form = RoomForm()
    return render(request, 'housing/add_room.html', {'form': form, 'residence': residence})

def add_rooms_bulk(request, residence_id):
    residence = Residence.objects.get(residence_ID=residence_id)

    if request.method == 'POST':
        room_data = request.POST.getlist('rooms')  # Assuming you're getting room data from a form
...
def manage_applications(request):
    ...
    applications = Application.objects.select_related('student').filter(student__isnull=False)
    ...
    if request.method == 'POST':
        ...
        admin_instance = get_object_or_404(HousingAdmin, user=request.user)  # Adjust based on your setup
    ...
    return render(request, 'housing/manage_applications.html', {
        'applications': applications,
    })
...
def assign_room(request, student_id):
    ...
    preferred_room_type = student.preferred_room_type  # Adjust based on your model structure
    ...
    if request.method == 'POST':
        ...
        if room.available_beds > 0:
            ...
            if current_assignments < room.bed_count:
                ...
                messages.success(request, f'Room assigned to {student.first_name} successfully.')
            else:
    ...
    return render(request, 'housing/assign_room.html', {'student': student, 'rooms': rooms})